...
public class RegistroPontoService {
    ...
    public RegistroPonto registrarPonto(Long funcionarioId) {
        ...
        Optional<RegistroPonto> ultimoRegistroOpt = registroPontoRepository.findByFuncionarioIdAndDia(funcionarioId,
                hoje);
        if (ultimoRegistroOpt.isPresent()) {
            ...
            LocalTime ultimoHorario = Stream.of(
                    ...
                    ultimoRegistro.getSaida()).filter(Objects::nonNull)
                    .max(Comparator.naturalOrder())
                    .orElse(null);

            // Se o último horário foi registrado há menos de 10 minutos, bloqueia o novo
            // registro
            if (ultimoHorario != null && Duration.between(ultimoHorario, agora).toMinutes() < 10) {
                throw new IllegalArgumentException(
                        "Registro não permitido. Aguarde 10 minutos antes de registrar novamente.");
            }
        }
    ...
    public Map<Long, List<RegistroPonto>> listarRegistrosAgrupadosPorFuncionarioMes(int ano, int mes, String funcao) {
        ...
        if (funcao != null && !funcao.isEmpty()) {
            List<Candidatos> candidatos = candidatosRepository.findByFuncao(funcao);
            List<String> prontuarios = new ArrayList<>();
            for (Candidatos c : candidatos) {
                prontuarios.add(c.getProntuario());
            }

            List<Interno> internos = internoRepository.findByProntuarioIn(prontuarios);
            List<Long> ids = new ArrayList<>();
            for (Interno i : internos) {
                ids.add(i.getId());
            }

            registros = registroPontoRepository.findByFuncionarioIdInAndDiaBetween(ids, inicio, fim);
        } else {
            registros = registroPontoRepository.findByDiaBetween(inicio, fim);
        }

        Map<Long, List<RegistroPonto>> agrupados = new HashMap<>();
        for (RegistroPonto rp : registros) {
            agrupados.computeIfAbsent(rp.getFuncionarioId(), k -> new ArrayList<>()).add(rp);
        }

        return agrupados;
    }

    public RegistroPonto salvarOuAtualizarRegistro(Long funcionarioId, Interno interno, LocalDate data,
            LocalTime horario) {
    ...
    // Novo método para listar registros agrupados por funcionário para um
    // determinado mês
    ...
    public List<RegistroPonto> listarRegistrosPorFuncionarioMes(Long funcionarioId, int ano, int mes) {
        ...
        List<RegistroPonto> registros = registroPontoRepository.findByFuncionarioIdAndDiaBetween(funcionarioId, inicio,
                fim);
     ...
     * - Se há 2 ou 3 registros: utiliza o intervalo do primeiro ao segundo
     * registro.
     * - Se há 4 registros: soma o intervalo (primeiro ao segundo) e (terceiro ao
     * quarto).
     */
